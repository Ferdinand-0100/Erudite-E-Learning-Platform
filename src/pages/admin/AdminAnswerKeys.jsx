...
export default function AdminAnswerKeys() {
  ...
  const [viewMode, setViewMode] = useAppState('admin-answerkeys-view-mode', 'public')

  useEffect(() => { fetchAnswerKeys() }, [courseKey, viewMode])
  ...
  async function fetchAnswerKeys() {
    setLoading(true)
    setError(null)
    let query = supabase.from('answer_keys').select('*').eq('is_private', viewMode === 'private').order('sort_order').order('created_at', { ascending: false })
  ...
  function startEdit(key) {
    setEditingId(key.id)
    setForm({ title: key.title, difficulty: key.difficulty || 'Beginner', tags: key.tags || [], is_private: key.is_private ?? false, week_number: key.week_number ?? 1 })
    setError(null)
  }
    ...
    <div style={{ padding: 'var(--space-6)', maxWidth: 900 }}>
      ...
      {viewMode === 'private' && (
        <div style={{ marginBottom: 'var(--space-4)', padding: '10px 14px', background: 'var(--color-surface-2)', border: '2px solid var(--color-border)', borderRadius: 'var(--radius-wobbly-sm)', fontSize: 13, color: 'var(--color-text-2)' }}>
          Private answer keys are not tied to a course. They can only be accessed through assigned Study Guides.
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} style={{ background: 'var(--color-surface)', border: '2px solid var(--color-border)', borderRadius: 'var(--radius-wobbly-sm)', padding: 'var(--space-4)', marginBottom: 'var(--space-6)', display: 'grid', gap: 'var(--space-3)', boxShadow: 'var(--shadow-card)' }}>
        <h2 style={{ fontSize: '15px', fontWeight: 600, margin: 0 }}>{editingId ? 'Edit Answer Key' : `Add ${viewMode === 'private' ? 'Private' : 'Public'} Answer Key`}</h2>
      ...
      {loading ? (
        ...
        <p style={{ color: 'var(--color-text-3)', fontSize: '14px' }}>No {viewMode} answer keys{viewMode === 'public' ? ' for this course key' : ''}.</p>
      ) : (
                ...
                <tbody>
                  {items.map(k => (
                    <SortableRow key={k.id} id={k.id}>
                      ...
                      {viewMode === 'public' && <td style={{ padding: '8px 10px', fontWeight: 600, color: 'var(--color-secondary)' }}>{k.week_number ?? 1}</td>}
